...
def main():
    nombre_jugador = input(
        "Bienvenido a la aventura de Dungeons and Dragons! Por favor, ingresa tu nombre: "
    )
    jugador = Jugador(nombre_jugador)

    enemigos = [
        Enemigo("Goblin", 20, 10),
        Enemigo("Zombie", 40, 20),
        Enemigo("Hombre Lobo", 60, 40),
    ]
    ...
    while enemigos:
        ...
        print(f"Te encuentras conm un {enemigo_actual.nombre} en tu camino")

        while enemigo_actual.salud > 0:
            accion = input("¿Qué haces? (atacar/huir): ").lower()
        ...
        if continuar != "s":
            print("¡Gracias por haber jugado Dungeons and Dragons!")
            break
...
if (
    __name__ == "__main__"
):  # nos asegura que solo podremos ejecutar el script desde el programa principal
    main()
